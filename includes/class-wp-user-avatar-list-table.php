...
class WP_User_Avatar_List_Table extends WP_List_Table {
	...
	public function get_views() {
		...
		$type_links['all'] = sprintf('<a href="%s">', esc_url(add_query_arg(array('page' => 'wp-user-avatar-library'), 'admin.php'))).sprintf('All <span class="count">(%s)</span>', $_total_posts).'</a>';
		return $type_links;
	}
			...
			<?php
						...
						?>
							<th scope="row" class="check-column">
								<?php if($user_can_edit) { ?>
									<label class="screen-reader-text" for="cb-select-<?php the_ID(); ?>"><?php echo sprintf('Select %s', $att_title);?></label>
						...
						case 'icon':
							...
							?>
								<td <?php echo $attributes ?>><?php
									...
									?>
										<a href="<?php echo get_edit_post_link($post->ID, true); ?>" title="<?php echo esc_attr(sprintf('Edit &#8220;%s&#8221;', $att_title)); ?>">
											<?php echo $thumb; ?>
										</a>
									<?php } }
									?>
				...
				<td <?php echo $attributes ?>><strong>
					...
					<a href="<?php echo get_edit_post_link($post->ID, true); ?>"
						title="<?php echo esc_attr(sprintf('Edit &#8220;%s&#8221;', $att_title)); ?>">
				...
				case 'date':
					if('0000-00-00 00:00:00' == $post->post_date) {
						...
						if ((abs($t_diff = time() - $time)) < DAY_IN_SECONDS) {
							$h_time = human_time_diff($time) . ( ($t_diff < 0) ? ' from now' : ' ago');
						} else {
	...
	public function _get_row_actions($post, $att_title) {
		...
		if(current_user_can('edit_post', $post->ID) && !$this->is_trash) {
			$actions['edit'] = '<a href="'.get_edit_post_link($post->ID, true).'">Edit</a>';
		}
		if(current_user_can('delete_post', $post->ID)) {
			if($this->is_trash) {
				$actions['untrash'] = "<a class='submitdelete' href='".wp_nonce_url("post.php?action=untrash&amp;post=$post->ID", 'untrash-post_'.$post->ID)."'>Restore</a>";
			} elseif (EMPTY_TRASH_DAYS && MEDIA_TRASH) {
				$actions['trash'] = "<a class='submitdelete' href='".wp_nonce_url("post.php?action=trash&amp;post=$post->ID", 'trash-post_'.$post->ID)."'>Trash</a>";
			}
			if($this->is_trash || !EMPTY_TRASH_DAYS || !MEDIA_TRASH) {
				$delete_ays = (!$this->is_trash && !MEDIA_TRASH) ? " onclick='return showNotice.warn();'" : "";
				$actions['delete'] = "<a class='submitdelete'$delete_ays href='".wp_nonce_url("post.php?action=delete&amp;post=$post->ID", 'delete-post_'.$post->ID)."'>Delete Permanently</a>";
			}
		}
		if(!$this->is_trash) {
			$title = _draft_or_post_title($post->post_parent);
			$actions['view'] = '<a href="'.get_permalink($post->ID).'" title="'.esc_attr(sprintf('View &#8220;%s&#8221;', $title)).'" rel="permalink">View</a>';
		}
		return $actions;
	}
}
